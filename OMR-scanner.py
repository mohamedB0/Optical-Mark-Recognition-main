...
class AutoGraderApp:
    def __init__(self):
        ...
        self.questions = 20
        self.choices = 20
        self.ans = [2, 3, 4, 1, 2, 3, 3, 2, 2, 3, 1, 3, 4, 3, 2, 1, 1, 4, 3, 2]  # Correct answers
        self.count = 0
        ...
        self.text_label = Label(self.intro_frame, text="Welcome to AutoGrader!", font=("Arial", 20), bg="lightblue")
        self.text_label.pack(pady=20)

        self.description_label = Label(self.intro_frame,
                                       text="AutoGrader is an application for automatic grading of test answers. "
                                            "You can use a camera or upload an image of the test.",
                                       font=("Arial", 14), bg="lightblue", wraplength=400, justify="center")
        self.description_label.pack(pady=10)

        self.select_label = Label(self.intro_frame, text="Select an option:", font=("Arial", 16), bg="lightblue")
        self.select_label.pack()

        self.camera_button = Button(self.intro_frame, text="Use Camera", command=self.use_camera,
                                    font=("Arial", 14), bg="lightgreen", fg="black")
        self.camera_button.pack(pady=10)

        self.photo_button = Button(self.intro_frame, text="Upload Image", command=self.upload_photo, font=("Arial", 14),
        ...
        self.header_label = Label(self.questions_frame, text="Set the correct answers:", font=("Arial", 12),
                                  bg="lightblue")
        self.header_label.pack(pady=10)

        self.options = []
        for i in range(self.questions):
            question_label = Label(self.questions_frame, text=f"Question {i + 1}:", font=("Arial", 12), bg="lightblue")
            ...
            for j in range(self.choices):
                option_button = Radiobutton(option_frame, text=f"Answer {j + 1}", font=("Arial", 10),
        ...
        while True:
            ...
            try:
                ...
                if biggestPoints.size != 0 and gradePoints.size != 0:
                    ...
                    # DISPLAYING ANSWERS
                    utils.showAnswers(imgWarpColored, myIndex, grading, self.ans)  # DRAW DETECTED ANSWERS
                    utils.drawGrid(imgWarpColored)  # DRAW GRID
                    imgRawDrawings = np.zeros_like(imgWarpColored)  # NEW BLANK IMAGE WITH WARP IMAGE SIZE
                    utils.showAnswers(imgRawDrawings, myIndex, grading, self.ans)  # DRAW ON NEW IMAGE
